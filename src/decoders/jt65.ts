/** WebSocket client for the server-side JT65 decoder.
 *
 *  JT65 runs in 1-minute UTC-aligned slots, decoded by the same `jt9`
 *  binary with the `-65` flag. Wire protocol matches the JT9 client. */

import type { Jt9Spot } from './jt9';

export type Jt65Spot = Jt9Spot;

export interface Jt65DecoderOpts {
  sampleRate: number;
  dialKHz?: number;
  onSpot?: (s: Jt65Spot) => void;
  onStatus?: (s: string) => void;
}

export class Jt65Decoder {
  private ws: WebSocket | null = null;
  private opts: Jt65DecoderOpts;
  private queue: Int16Array[] = [];

  constructor(opts: Jt65DecoderOpts) {
    this.opts = opts;
    this.connect();
  }
  ...
  private connect() {
    ...
    const url = `${proto}//${location.host}/ws/decode/jt65${q.toString() ? '?' + q.toString() : ''}`;
    ...
    ws.onmessage = (e) => {
      ...
      if (msg.t === 'spot') this.opts.onSpot?.(msg as Jt65Spot);
