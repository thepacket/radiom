...
// WWV scope frame: emit "WV" + uint16-LE count + count uint8 samples (already
// normalised to 0..255 by wwv::update_syncscope before this is called).
void set_video(double* p, int n) {
  if (n <= 0 || n > 65535) return;
  unsigned char hdr[4];
  hdr[0] = 'W'; hdr[1] = 'V';
  hdr[2] = (unsigned char)(n & 0xff);
  hdr[3] = (unsigned char)((n >> 8) & 0xff);
  std::fwrite(hdr, 1, 4, stdout);
  for (int i = 0; i < n; i++) {
    double v = p[i];
    if (v < 0) v = 0; else if (v > 255) v = 255;
    unsigned char b = (unsigned char)v;
    std::fwrite(&b, 1, 1, stdout);
  }
  std::fflush(stdout);
}
