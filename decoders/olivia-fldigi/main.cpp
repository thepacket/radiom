// Driver: reads 12 kHz int16 PCM from stdin, resamples to 8 kHz (Olivia's
// internal rate), feeds the vendored fldigi olivia modem, and lets
...
// Look up the fldigi mode enum for a (tones, bandwidth) pair. Falls back to
// MODE_OLIVIA (default 32/1000) if the combo isn't a fldigi-known preset.
trx_mode olivia_mode_for(int tones, int bw) {
  struct M { int t, b; trx_mode m; };
  static const M map[] = {
    {  4,  125, MODE_OLIVIA_4_125  },
    {  4,  250, MODE_OLIVIA_4_250  },
    {  4,  500, MODE_OLIVIA_4_500  },
    {  4, 1000, MODE_OLIVIA_4_1000 },
    {  4, 2000, MODE_OLIVIA_4_2000 },
    {  8,  125, MODE_OLIVIA_8_125  },
    {  8,  250, MODE_OLIVIA_8_250  },
    {  8,  500, MODE_OLIVIA_8_500  },
    {  8, 1000, MODE_OLIVIA_8_1000 },
    {  8, 2000, MODE_OLIVIA_8_2000 },
    { 16,  500, MODE_OLIVIA_16_500 },
    { 16, 1000, MODE_OLIVIA_16_1000},
    { 16, 2000, MODE_OLIVIA_16_2000},
    { 32, 1000, MODE_OLIVIA_32_1000},
    { 32, 2000, MODE_OLIVIA_32_2000},
    { 64, 500,  MODE_OLIVIA_64_500 },
    { 64, 1000, MODE_OLIVIA_64_1000},
    { 64, 2000, MODE_OLIVIA_64_2000},
  };
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (std::strncmp(a, "--smargin=", 10) == 0) progdefaults.oliviasmargin = std::atoi(a + 10);
    else if (std::strncmp(a, "--sinteg=",   9) == 0) progdefaults.oliviasinteg  = std::atoi(a +  9);
  }
  if (carrierHz < 200.0)  carrierHz = 200.0;
  if (carrierHz > 3000.0) carrierHz = 3000.0;

  trx_mode m = olivia_mode_for(tones, bw);
  // Set the carrier explicitly — olivia.cxx::restart() reads frequency to
  // place the first MFSK tone, with progdefaults.StartAtSweetSpot==false
  // so that it honours the value we set.
  progdefaults.StartAtSweetSpot = false;

  olivia* modem_ol = new olivia(m);
  active_modem = modem_ol;
  modem_ol->init();
  modem_ol->set_freq(carrierHz);
  modem_ol->rx_init();
  ...
  if (gen) {
    ...
    if (modem* mm = active_modem) {
      ...
      std::fflush(stdout);
      std::fprintf(stderr, "[olivia:gen] samples=%zu rate=%d ticks=%d\n",
  ...
  for (;;) {
    ...
    if (!out8k.empty()) modem_ol->rx_process(out8k.data(), int(out8k.size()));
  }
  return 0;
}
