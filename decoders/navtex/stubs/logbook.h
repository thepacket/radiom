...
enum {
  ...
  QTH, COUNTRY, GRIDSQUARE, XCHG1, SRX,
};

// ADIF NOTES line separator (fldigi defines as macro).
#ifndef ADIF_EOL
#define ADIF_EOL "\r\n"
#endif

// fldigi's QsoHelper RAII wrapper that builds + saves a cQsoRec on
// destruction. RX-only build → no-op.
class QsoHelper {
public:
  explicit QsoHelper(int /*mode*/) {}
  ~QsoHelper() {}
  void Push(int, const char*)        {}
