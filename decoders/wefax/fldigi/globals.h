...
// (We drop the `const` here vs. fldigi's stock globals.h so our
// fldigi_glue.cpp can populate the table at startup without writing
// into a read-only segment. fldigi never mutates mode_info at runtime,
// only at static-init time, so the API contract is preserved.)
