...
// fldigi modems read wf->USB(), wf->Reverse(), wf->Carrier() during
// rx_process. Pointing wf at a real instance (with sensible defaults
// — USB=true, Reverse=false, Carrier=1500) avoids the null-deref that
// produced garbled output on every fldigi-vendored decoder.
static waterfall wf_instance;
waterfall*       wf           = &wf_instance;
...
void set_scope(double*, int)         {}
void activate_mfsk_image_item(bool)  {}
void start_deadman()                 {}
void stop_deadman()                  {}
int  load_image(const char*)         { return 0; }
void pic_TxSendGrey()                {}
void pic_TxSendColor()               {}
double xmtpicbuff[1] = {0};
std::string PicsDir = "";
...
// DominoEX hooks (also used by THOR).
void put_sec_char(int)              {}
void set_video(double*, int, bool)  {}

// RTTY scope helper — RX-only build doesn't drive the phase scope.
void set_zdata(void*, int)          {}

// FSQ-specific GUI/log hooks.
void display_fsq_rx_text(const std::string& s, int /*style*/) {
  // The decoded text path. Stream printable chars to stdout so the Node
  // wrapper can forward them to the WS client.
  std::fwrite(s.data(), 1, s.size(), stdout);
  std::fflush(stdout);
}
void display_fsq_mon_text(const std::string&, int) {}
void show_notifier(notify_dialog*)                  {}
void close_fsqMonitor()                             {}
void enableSELCAL()                                 {}
void add_to_heard_list(const std::string&, const std::string&) {}
void fsq_showRxViewer(int, int, const std::string&) {}
void fsq_updateRxPic(unsigned char, int)            {}
void fsq_updateTxPic(unsigned char, int)            {}
void fsq_clear_rximage()                            {}
void fsq_clear_tximage()                            {}
void fsq_enableshift()                              {}
void write_fsq_que(const char*)                     {}
void write_fsq_que(const std::string&)              {}

std::string TempDir = "";
std::string zdate()     { return ""; }
std::string ztime()     { return ""; }
std::string zshowtime() { return ""; }

FsqStubBtn _fsqBtnSelcal, _fsqBtnOffsetOn;
FsqStubVal _fsqCtrlOffset;
FsqStubBtn* btn_SELCAL       = &_fsqBtnSelcal;
FsqStubBtn* btnOffsetOn      = &_fsqBtnOffsetOn;
FsqStubVal* ctrl_freq_offset = &_fsqCtrlOffset;
FsqPicWin   _fsqPicTxWin;
FsqPicWin*  fsqpicTxWin   = &_fsqPicTxWin;
FsqSelStub  _fsqSelSize;
FsqSelStub* selfsqpicSize = &_fsqSelSize;
FsqTxText   _fsqTxText;
FsqTxText*  fsq_tx_text   = &_fsqTxText;
...
// ── mode_info table ────────────────────────────────────────────────────
// PSK reads mode_info[mode].sname for status text. Initialise all entries
// with empty strings (never null, since std::string += chokes), then fill
// in the PSK rows we actually use.
...
struct ModeInfoBootstrap {
  ModeInfoBootstrap() {
    ...
    auto fill = [](int idx, const char* s, const char* n) {
      mode_info[idx].sname = s;
      mode_info[idx].name  = n;
      mode_info[idx].adif_name      = "RTTY";
      mode_info[idx].pskmail_name   = s;
      mode_info[idx].export_mode    = "RTTY";
      mode_info[idx].export_submode = s;
      mode_info[idx].vid_name       = s;
    };
    fill(MODE_RTTY, "RTTY", "RTTY");
  }
...
namespace { synop _synop_singleton; }
synop* synop::instance() { return &_synop_singleton; }
