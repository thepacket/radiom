...
// MT63 build: stub cMorse so the modem ctor can `new cMorse` without
// dragging in morse.cxx (which depends on CW-only progdefaults fields).
class cMorse {
public:
	cMorse() {}
	~cMorse() {}
	...
	std::string rx_lookup(std::string) { return ""; }
	std::string tx_lookup(int) { return ""; }
	std::string tx_print() { return ""; }
	int  tx_length(int) { return 0; }
};
